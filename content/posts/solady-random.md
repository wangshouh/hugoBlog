...
此处的 `result` 就是使用哈希算法获取的第一个随机数，同时也作为后期 `MCG` 的种子。此处，我们需要注意 `MCG` 的参数选择，其 `n` 值选择了一个 `uint256` 内的一个较大的素数。
...
`, sum)` 的结果精度为 $2^{96} \times 10^{18}$ ，我们可以使用右移 96 位的方式使其精度修正为 $10^{18}$
