...
首先讨论对非法指针解引用的解决方案，Sierra 引入了 `Box` 作为指针的类型，`Box<T>` 类型代表一个初始化过的有效的指针类型，而 `box_new<T> -> Box<T>` 函数用于生成一个指针，`box_deref(Box<T>) -> T` 函数用于解引用。基于上述 `Box<T>` 类型及其函数，我们实现了指针的始终有效性。因为创建指针只能通过 `box_new<T>` 实现，该函数可以保证指针是经过初始化的。
