...
在上文中，我们在`setup`和`execTransaction`中都使用了这一重要的参数。我们会在此处详细介绍此函数的参数和代码逻辑。由于此处使用到了`gas`相关的基础知识，特别是`EIP1559`相关的`gas`机制，建议读者先行阅读[此文](../ethereum-gas)

我们首先从参数分析，此函数需要以下参数:

- gasUsed 用于计算手续费的`gas`数值
- baseGas 类似`EIP1559`中的`Base Fee`，具体可以参考[此文](../ethereum-gas#base-fee)
- gasPrice `gas`的价格
...
此函数用于编码交易数据，此函数使用的参数我们在前文都进行过相关介绍。对于此函数，我们不会进行详细介绍，读者可以参考我之前的博客[基于链下链上双视角深入解析以太坊签名与验证](../ecsda-sign-chain#eip712)
