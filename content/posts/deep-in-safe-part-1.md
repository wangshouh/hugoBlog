...
值得注意的是中继商进行上述操作需要合约支持，比较著名的有`EIP2771 MetaTranscation`标准，具体可以参考[EIP712的扩展使用](https://hugo.wongssh.cf/posts/eip712-extend/#meta-transactions)。当然，`Gnosis Safe`合约对于中继商进行交易进行了很好的支持，我们会在下文逐渐介绍。
...
> 注意上述表述为创建代码而不是运行代码，具体请参考[此文章](https://hugo.wongssh.cf/posts/deep-in-eip1167/#%E5%88%9D%E5%A7%8B%E5%8C%96)
...
如果读者无法理解两者的区别，请参考[此文章](https://hugo.wongssh.cf/posts/deep-in-eip1167/#%E5%88%9D%E5%A7%8B%E5%8C%96)
...
此部分可以参考我之前写的[使用多种方式编写可升级的智能合约(上)](https://hugo.wongssh.cf/posts/foundry-contract-upgrade-part1/#%E5%8E%9F%E7%90%86%E8%A7%A3%E6%9E%90)。
...
- [基于链下链上双视角深入解析以太坊签名与验证](https://hugo.wongssh.cf/posts/ecsda-sign-chain/)

- [EIP712的扩展使用](https://hugo.wongssh.cf/posts/eip712-extend/)
...
在上文中，我们在`setup`和`execTransaction`中都使用了这一重要的参数。我们会在此处详细介绍此函数的参数和代码逻辑。由于此处使用到了`gas`相关的基础知识，特别是`EIP1559`相关的`gas`机制，建议读者先行阅读[此文](https://hugo.wongssh.cf/posts/ethereum-gas)

我们首先从参数分析，此函数需要以下参数:

- gasUsed 用于计算手续费的`gas`数值
- baseGas 类似`EIP1559`中的`Base Fee`，具体可以参考[此文](https://hugo.wongssh.cf/posts/ethereum-gas#base-fee)
- gasPrice `gas`的价格
...
- dataHash 交易参数的`EIP712`哈希值，具体可以参考[此文章](https://hugo.wongssh.cf/posts/ecsda-sign-chain/#%E7%AD%BE%E5%90%8D-1)
- data 需要进行签名的数据
...
> 此处的常数为`65`的原因是在最小签名(即仅包含`v`、`r`、`s`)的长度为`65 bytes`。具体可以参考[此文章](https://hugo.wongssh.cf/posts/ecsda-sign-chain/#%E4%BB%A5%E5%A4%AA%E5%9D%8A%E4%BA%A4%E6%98%93%E7%AD%BE%E5%90%8D)。
...
读者在阅读此部分时需要对`EIP1271`标准有一定理解，如果读者对此没有了解，请先阅读[此文章](https://hugo.wongssh.cf/posts/ecsda-sign-chain/#eip-1271)。简单来说，合约将签名权授予某拥有私钥的用户，由此用户进行签名。接受合约签名的合约使用合约签名对`签名验证合约`调用`isValidSignature`函数，获得此签名是否是正确的合约签名。
...
此函数用于编码交易数据，此函数使用的参数我们在前文都进行过相关介绍。对于此函数，我们不会进行详细介绍，读者可以参考我之前的博客[基于链下链上双视角深入解析以太坊签名与验证](https://hugo.wongssh.cf/posts/ecsda-sign-chain#eip712)
